...
def test_1_create():
    response = client.post("/expenses", json={"amount": 100.0, "category": "food", "description": "Groceries", "spent_on": str(date.today())})
    assert response.status_code == 201

def test_2_create_invalid_amount():
    response = client.post("/expenses", json={"amount": -5.0, "category": "food", "description": "Invalid", "spent_on": str(date.today())})
    assert response.status_code == 422

def test_3_create_invalid_category():
    response = client.post("/expenses", json={"amount": 50.0, "category": "unknown", "description": "Invalid", "spent_on": str(date.today())})
    assert response.status_code == 422
...
def test_5_list_filtered():
    response = client.get("/expenses?category=food")
    assert response.status_code == 200

def test_6_get_by_id():
    response = client.get("/expenses/1")
    assert response.status_code == 200
    assert response.json()["id"] == 1

def test_7_get_by_id_not_found():
    response = client.get("/expenses/9999")
    assert response.status_code == 404

def test_8_update():
    response = client.patch("/expenses/1", json={"amount": 120.0})
    assert response.status_code == 200
